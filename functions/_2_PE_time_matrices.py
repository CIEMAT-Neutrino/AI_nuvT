...
def process_photoelectrons2(hit_PE, hit_ch, hit_PE2, hit_ch2, hit_t, n_canales=312):
    """
    Process the filtered hit data to compute the number of photoelectrons and weighted times.
    ...
    hit_ch2 (list of awkward arrays): Filtered channel data, it goes with hit_t.
    hit_t (list of awkward arrays): Filtered time data.
    ...
    for i in range(n_events):
        for j in range(len(hit_PE[i])):
            for k, l in zip(hit_PE[i][j], hit_ch[i][j]):
                pe_matrix[i][l] += k

    # falta calcular time_matric utilizando hit_ch2 y hit_t

    # Populate the matrices
    for i in range(n_events):
        for j in range(len(hit_PE2[i])):
            for k, l, t in zip(hit_PE2[i][j], hit_ch2[i][j], hit_t[i][j]):
                time_matrix[i][l] += k * t  # Weight times by charge
